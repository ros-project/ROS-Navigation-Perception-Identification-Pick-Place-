	  ...
	  for(int i=0;i<x;i++){
		...
		if(i==0){
		  ...
		  if(centroid0[2]>=0.25)
		  {
			...
			mp[i]=V;
			vec.emplace_back(centroid0[2],i);
		  }
		...
		if(i==1){
		  ...
		  if(centroid1[2]>=0.25)
		  {
			...
			mp[i]=V;
			vec.emplace_back(centroid1[2],i);
		  }
		...
		if(i==2){
		  ...
		  if(centroid2[2]>=0.25)
		  {
			...
			mp[i]=V;
			vec.emplace_back(centroid2[2],i);
		  }
		...
		if(i==3){
		  ...
		  if(centroid3[2]>=0.25)
		  {
			...
			mp[i]=V;
			vec.emplace_back(centroid3[2],i);
		  }
		...
		if(i==4){
		  ...
		  if(centroid4[2]>=0.25)
		  {
			...
			mp[i]=V;
			vec.emplace_back(centroid4[2],i);
		  }
		...
		if(i==5){
		  ...
		  if(centroid5[2]>=0.25)
		  {
			...
			mp[i]=V;
			vec.emplace_back(centroid5[2],i);
		  }
		...
		if(i==6){
		  ...
		  if(centroid6[2]>=0.25)
		  {
			...
			mp[i]=V;
			vec.emplace_back(centroid6[2],i);
		  }
		...
		if(vec.size()==2)
		{
			...
			some_object.x1=mp[vec[0].second][0];
			some_object.y1=mp[vec[0].second][1];
			some_object.z1=mp[vec[0].second][2];
	...
	*cloud_filtered = *cloud_f;
	...
	for (std::vector<pcl::PointIndices>::const_iterator it = cluster_indices.begin (); it != cluster_indices.end (); ++it)
	{
	  ...
	  for(int i=0;i<x;i++){
		...
		if(ind<=3){
			int c=ind-1;
			std::sort(mp_ans.rbegin(),mp_ans.rend());
			for(auto i:mp_ans){
				std::cout<<i.first<<" "<<i.second<<std::endl;
			}
			int ind_help=mp_ans[c].second;
			task5_msgs::Data_msg some_object;
	        some_object.x3=mp[ind_help][0];
	        some_object.y3=mp[ind_help][1];
	        some_object.z3=mp[ind_help][2];
	        task5_msg.publish(some_object);
		}
		std::cout<<ind<<std::endl;
		if(ind>=4&&ind<=6){
			int c=7,ind_help;
			std::sort(mp_ans.begin(),mp_ans.end());
			for(auto i:mp_ans){
				c--;
				std::cout<<i.first<<" "<<i.second<<std::endl;
				if(c==ind){
